 ...
 * mapped exactly like the dynamic [com.berberoglus.sbnetworking.HttpClient.executeRaw] path.
 */
internal class HttpClientCallAdapterFactory : CallAdapter.Factory() {
    override fun get(returnType: Type, annotations: Array<Annotation>, retrofit: Retrofit): CallAdapter<*, *>? {
        if (getRawType(returnType) != Call::class.java) return null
        val responseType = getParameterUpperBound(0, returnType as ParameterizedType)
        return object : CallAdapter<Any, Call<*>> {
            override fun responseType(): Type = responseType
            override fun adapt(call: Call<Any>): Call<*> = NormalizingCall(call)
        }
    }
}

private class NormalizingCall<T>(private val delegate: Call<T>) : Call<T> {
    override fun enqueue(callback: Callback<T>) {
        delegate.enqueue(object : Callback<T> {
            ...
            override fun onFailure(call: Call<T>, t: Throwable) {
                callback.onFailure(call, if (t is IOException) mapIo(t) else t)
            }
        })
    }
    override fun clone(): Call<T> = NormalizingCall(delegate.clone())
